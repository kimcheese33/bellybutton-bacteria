...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var samplesArrayAll = data.samples;

    // 4. Create a variable that filters the samples for the object with the desired sample number.
    var samplesArray = samplesArrayAll.filter(sampleObject => sampleObject.id == sample);

    // 1. Create a variable that filters the metadata array for the object with the desired sample number.
    var metadataArray = data.metadata.filter(sampleObject => sampleObject.id == sample);
    
    //  5. Create a variable that holds the first sample in the array.
    var chosenSample = samplesArray[0];
    var chosenMetadata = metadataArray[0];

    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    var otuID = chosenSample.otu_ids;
    var otuLabels = chosenSample.otu_labels;
    var sampleValues = chosenSample.sample_values;

    //Create a variable that holds the washing frequency.
    var washFreq = chosenMetadata.wfreq

    // 7. Create the yticks for the bar chart.
    // Hint: Get the the top 10 otu_ids and map them in descending order  
    //  so the otu_ids with the most bacteria are last. 

    var yticks = otuID.map(ID => ID).slice(0,10).reverse();
